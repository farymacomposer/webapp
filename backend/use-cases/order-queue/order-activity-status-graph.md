...
- По целевому правилу прямых переходов из-за `StreamCanceled` нет: сначала заказы `Preorder` и `Pending`, связанные с отменяемым стримом, должны быть перенесены на другой стрим.
...
- Целевое правило требует запрещать переход `Scheduled -> InProgress`, но в текущей реализации `ReviewOrderService.TakeInProgress()` это ограничение явно не проверяется. Сейчас код проверяет только `ReviewOrder.Status == Pending`, `!IsFrozen`, наличие `Live` стрима и отсутствие другого `InProgress`.
- По целевому правилу `StreamCanceled` не должен напрямую менять активность заказов, пока связанные `Preorder` и `Pending` заказы не перенесены на другой стрим.
- Текущая реализация этому правилу не соответствует полностью:
  - `ComposerStreamService.Cancel()` не переносит такие заказы автоматически и отмена блокируется до переноса;
  - обработка `StreamCanceled` в `OrderQueueService` сейчас сводится к полному пересчету очереди, а не к отдельному сценарию миграции заказов.
